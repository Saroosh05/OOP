...
using Bussiness_Application_Classes.BL;
...
namespace Business_Application
{
    internal class Program
    {
        ...
        {
            // Lists for storing data about officers, admins, complainants, and other entities.

            // List of all the police officers that can signin
            List<Police_officer> officers = new List<Police_officer>();

            Police_officer officer1 = new Police_officer("Rahim", "PC-123", "Inspector");
            Police_officer officer2 = new Police_officer("Farhan Shah", "PC-456", "Inspector");
            Police_officer officer3 = new Police_officer("Bilal Ahmad", "PC-789", "Inspector");
            ...
            // List that contains all the user names, passwords and roles that are signed-up
            ...
            // List storing information of complaints
            List<Complaint> complaints = new List<Complaint>();

            // List containing complainant's data
            List<Complainant> complainants = new List<Complainant>();
            // total number of complainants
            int complainantCount = 0;


            //int k = 0;


            // List saving signed-up police officers
            List<SignedUp_Officer> signedUpOffs = new List<SignedUp_Officer>();
            // number of signed-up officers
            int signedoffCount = 0;


            // List being used for assigning complaints to police officers
            List<Complaint_Assign> assignComps = new List<Complaint_Assign>();
            string assignToCode = "";    
            int assignCount = 0;


            // List being used for saving status of complaints
            ...
            while (true)
            {
                ...
                if (option == "1")
                {
                    ...
                    string role = signIn(users, user, pass, countUser);
                    ...
                    else
                    {
                        ...
                        if (role == "3")
                        {
                            ...
                            int compIndex;
                            int comparison = compCompare(user, pass, complainants, complainantCount);
                            ...
                            if (comparison == -1)
                            {
                                ...
                                complaints[compIndex].compName = "";
                                complaints[compIndex].email = "";
                                complaints[compIndex].contact = "";
                                complaints[compIndex].address = "";
                                complaints[compIndex].city = "";
                                complaints[compIndex].province = "";
                                complaints[compIndex].date = "";
                                complaints[compIndex].subject = "";
                                complaints[compIndex].urgencyLevel = "";
                                complaints[compIndex].detail = "";
                            }
                            else
                            {
                            ...
                            while (true)
                            {
                                ...
                                else if (requirement == "2")
                                {
                                    ...
                                    {
                                        //Complaint complnt = complaints[compIndex];
                                        
                                        if (complaints[compIndex].code == null || complaints[compIndex].code == "")
                                        {
                                            Console.SetCursorPosition(76, 22);
                                            Console.Write("SORRY! NO RECORD FOUND.");
                                        }
                                        else
                                        {
                                            ...
                                            string FurtherOpen = complaintsList(complaints, complaintCount, compIndex, ref loopVar, o, p);
                                            if (FurtherOpen != "0")
                                            {
                                                // The codeCompare function finds the index of the specific complaint on the basis of the provided code.
                                                int codeIndex = codeCompare(FurtherOpen, complaints, compIndex, complaints[compIndex]);
                                                if (codeIndex != -1)
                                                {
                                                    ...
                                                    if (tasks == "1")
                                                    {
                                                        ...
                                                        string deletedCode = compareIndex(complaints[compIndex].code, newString, codeIndex);

                                                        Complaint dlt = deleteComplaint(complaints, codeIndex, compIndex);
                                                        //complaints.Add(dlt);

                                                        complaintCount--;
                                                        codeIndex--;
                                                        complainants[compIndex].complaints = complaintCount;

                                                        Status st = new Status(null, null);
                                                        status.Add(st);

                                                        statusCount--;

                                                        int indexForAssigned = findIndexForAssign(deletedCode, assignComps, assignCount);
                                                        if (indexForAssigned != -1)
                                                        {
                                                            Complaint_Assign compAssign = new Complaint_Assign(null, null);
                                                            assignComps.Add(compAssign);
                                                            assignCount--;
                                                        }
                                                    ...
                                                    else if (tasks == "2")
                                                    {
                                                        ...
                                                        string currentCode = compareIndex(complaints[compIndex].code, newString, codeIndex);
                                                        if (currentCode != "nill")
                                                        {
                                                            // findIndex function finds the index of a specific string in an array.
                                                            int indexForStatus = findIndexForStatus(currentCode, status, statusCount);

                                                            if (status != null && indexForStatus >= 0 && status[indexForStatus] != null)
                                                            {
                                                                if (status[indexForStatus].status != "")
                                                                {
                                                                    compStatus = status[indexForStatus].status;
                                                                }
                                                            }
                                                        }
                                ...
                                else if (requirement == "3")
                                {
                                    ...
                                    Complaint addUp = complaint(complaints, compIndex, status, statusCount, complaintCount);
                                    complaints.Insert(compIndex,addUp);
                        ...
                        else if (role == "2")
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (requirement == "1")
                                {
                                    ...
                                    // findIndex function finds the index of a specific string in an array.
                                    int indexForOFFCode = findIndexForOffName(user, officers, officerCount);
                                    string OFFCode = officers[indexForOFFCode].code;

                                    List<int> officersIndexes = new List<int>();

                                    // Finding all the indexes of the complaints that have been assigned.
                                    int numberOfIndexes = findAllIndexesAssigned(OFFCode, assignComps, assignCount, officersIndexes);
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            codeOfComp = assignComps[officersIndexes[z]].assignedCompCodes;
                                            ...
                                            for (int i = 0; i < complainantCount; i++)
                                            {
                                                ...
                                                // The codeCompare function finds the index of the specific complaint on the basis of the provided code.
                                                indexInBTW = codeCompare(codeOfComp, complaints, i, complaints[i]);
                                                Console.WriteLine();
                                                if (indexInBTW != -1)
                                                {
                                                    complainantIndex = i;

                                                    string AssignedList = complaintsListForAssign(complaints, indexInBTW, complainantIndex, numberOfIndexes, ref loopVar, 22, 19 + z, j, ref serial);
                                                    endingY = 19 + z;
                                        ...
                                        {
                                            // The codeCompare function finds the index of the specific complaint on the basis of the provided code.
                                            int codeIndex = codeCompare(detailsForAssign, complaints, complainantIndex, complaints[complainantIndex]);
                                            if (codeIndex != -1)
                                            {
                                                Console.Clear();
                                                printHeader();
                                                ...
                                                generalDetails(complaints, codeIndex, complainantIndex, m, n);
                                                Console.SetCursorPosition(25, 23);
                                                Console.Write("Personal Information: ");

                                                int X = 23, Y = 25;
                                                // Displaying personal details of the assigned complaint
                                                personalDetails(complaints, codeIndex, complainantIndex, X, Y);
                                                ...
                                                if (yes == "Yes" || yes == "yes")
                                                {
                                                    ...
                                                    List<string> newString = new List<string>();
                                                    //The compareIndex function extracts and returns a code from a string on the basis of the given index.
                                                    string codeForStatus = compareIndex(complaints[complainantIndex].code, newString, codeIndex);

                                                    // findIndex function finds the index of a specific string in an array.
                                                    int indexForStatus = findIndexForStatus(codeForStatus, status, statusCount);
                                                    ...
                                                    if (statusAdmin != "nill")
                                                    {
                                                        if (statusAdmin == "1")
                                                        {
                                                            status[indexForStatus].status = "Pending";
                                                        }
                                                        else if (statusAdmin == "2")
                                                        {
                                                            status[indexForStatus].status = "Invalid";
                                                        }
                                                        else if (statusAdmin == "3")
                                                        {
                                                            status[indexForStatus].status = "Under Process";
                                                        }
                                                        else if (statusAdmin == "4")
                                                        {
                                                            status[indexForStatus].status = "Accomplished";
                                                        }

                                                        Console.SetCursorPosition(41, 29);
                                                        Console.Write("The status of the complaint under code " + codeForStatus + " is set as " + status[indexForStatus].status + ".");
                            ...
                            while (true)
                            {
                                ...
                                if (requirement == "1")
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        string NextToComps = complainantsAdmin(complainants, complainantCount);

                                        if (NextToComps != "0")
                                        {
                                            int index = 0;
                                            // checkForOff checks whether a string exists in an array or not
                                            bool existPass = checkForOff(NextToComps, complainants, complainantCount, ref index);
                                            if (!existPass)
                                            {
                                                Console.Clear();
                                                printHeader();
                                                ...
                                                if (complaints.Count == 0)
                                                {
                                                    ...
                                                    Console.Write("No complaints have been filed yet by the complainant " + complainants[index].complainant);
                                                }
                                                else
                                                {

                                                    if (complaints[index].code == null || complaints[index].code == "")
                                                    {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        Console.Write("Here is the list of complaints " + complainants[index].complainant + " have filed till now.");

                                                        int m = 22, n = 19;
                                                        // listisng all the complaints of a complainant
                                                        string FurtherOpen = complaintsList(complaints, complaintCount, index, ref loopVar, m, n);

                                                        if (FurtherOpen != "0")
                                                        {
                                                            int codeIndex = codeCompare(FurtherOpen, complaints, index, complaints[index]);
                                                            if (codeIndex != -1)
                                                            {
                                                                ...
                                                                int i = 30, j = 17;
                                                                generalDetails(complaints, codeIndex, index, i, j);
                                                                Console.SetCursorPosition(25, 23);
                                                                Console.Write("Personal Information: ");

                                                                int X = 23, Y = 25;
                                                                // displaying personal information
                                                                personalDetails(complaints, codeIndex, index, X, Y);
                                                                string tasks = historyAdminMenu(task);

                                                                List<string> newString = new List<string>();
                                                                string requiredCode = compareIndex(complaints[index].code, newString, codeIndex);
                                                                ...
                                                                {
                                                                    List<string> NewString = new List<string>();
                                                                    string deletedCode = compareIndex(complaints[index].code, NewString, codeIndex);

                                                                    Complaint dlt = deleteComplaint(complaints, codeIndex, codeIndex);

                                                                    /*deleteComplaintt(complaints[index].subject, codeIndex);
                                                                    deleteComplaintt(complaints[index].compName, codeIndex);
                                                                    deleteComplaintt(complaints[index].email, codeIndex);
                                                                    deleteComplaintt(complaints[index].contact, codeIndex);
                                                                    deleteComplaintt(complaints[index].address, codeIndex);
                                                                    deleteComplaintt(complaints[index].city, codeIndex);
                                                                    deleteComplaintt(complaints[index].province, codeIndex);
                                                                    deleteComplaintt(complaints[index].detail, codeIndex);
                                                                    deleteComplaintt(complaints[index].code, codeIndex);
                                                                    deleteComplaintt(complaints[index].urgencyLevel, codeIndex);
                                                                    deleteComplaintt(complaints[index].date, codeIndex);*/

                                                                    int indexForAssigned = findIndexForAssign(deletedCode, assignComps, assignCount);
                                                                    if (indexForAssigned != -1)
                                                                    {
                                                                        assignComps[indexForAssigned].assignedOffCode = "";
                                                                        assignComps[indexForAssigned].assignedCompCodes = "";
                                                                        assignCount--;
                                                                    }
                                                                    ...
                                                                    complaintCount--;
                                                                    codeIndex--;
                                                                    complainants[index].complaints = complaintCount;
                                                                ...
                                                                else if (tasks == "2")
                                                                {
                                                                    ...
                                                                    string mainString = findMainString(editNum, complaints, codeIndex, index, complaintCount, mainArr, ref editedString);
                                                                    ...
                                                                    storeInMain(editedString, mainArr, complaints, index);
                                                                ...
                                                                else if (tasks == "3")
                                                                {
                                                                    ...
                                                                    // findIndex function finds the index of a specific string in an array.
                                                                    int indexForStatus = findIndexForStatus(requiredCode, status, statusCount);
                                                                    ...
                                                                    if (statusAdmin != "nill")
                                                                    {
                                                                        ...
                                                                        Console.SetCursorPosition(41, 29);
                                                                        Console.Write("The status of the complaint under code " + requiredCode + " is set as " + status[indexForStatus].status + ".");
                                                                ...
                                                                {
                                                                    // findIndex function finds the index of a specific string in an array.
                                                                    int alreadyAssigned = findIndexForAssign(requiredCode, assignComps, assignCount);
                                                                    ...
                                                                    else
                                                                    {
                                                                        ...
                                                                        int length = signedOfficersList(signedUpOffs, officers, signedoffCount, officerCount, 41, 20);
                                                                        ...
                                                                        if (signedoffCount != 0)
                                                                        {
                                                                            ...
                                                                            int codeInd = findIndexForSignOff(assignToCode, signedUpOffs, signedoffCount);
                                                                            ...
                                                                            else
                                                                            {
                                                                                ...
                                                                                Complaint_Assign compAssi = new Complaint_Assign(requiredCode, assignToCode);
                                                                                assignComps.Add(compAssi);
                                                                                ...
                                                                                Console.Write("The complaint under the code " + requiredCode + " is assigned to the " + officers[codeInd].designation + " " + officers[codeInd].name + " with code " + assignToCode + ".");
                                                                            }
                                                                        }
                                                                    }
                                                                }
                                ...
                                else if (requirement == "2")
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        length1 = officersList(officers, officerCount, 41, 18);
                                    }

                                    Console.SetCursorPosition(29, 24 + length1);
                                    // displaying the list of officers who have signed up
                                    Console.Write("List of the officers that have signed up :");
                                    int length2 = signedOfficersList(signedUpOffs, officers, signedoffCount, officerCount, 41, 26 + length1);
                                    ...
                                    if (Option != "nill")
                                    {
                                        ...
                                        if (Option == "1")
                                        {
                                            if (length2 != 0)
                                            {
                                                ...
                                                int IfExist = findIndexForSignOff(codeToView, signedUpOffs, signedoffCount);
                                                ...
                                                else
                                                {
                                                    ...
                                                    int numberOfIndexes = findAllIndexesAssigned(codeToView, assignComps, assignCount, officersIndexes);
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        if (detailsForAssign != "0")
                                                        {
                                                            ...
                                                            if (codeIndex != -1)
                                                            {
                                                                ...
                                                                // Displaying General Details of an assigned complaint
                                                                int i = 30, l = 17;
                                                                generalDetails(complaints, codeIndex, complainantIndex, i, l);
                                                                Console.SetCursorPosition(25, 23);
                                                                Console.Write("Personal Information: ");
                                        ...
                                        else if (Option == "2")
                                        {
                                            ...
                                            int checkingExistance = findIndexForOff(code, officers, officerCount);
                                            ...
                                            else
                                            {
                                                ...
                                                Police_officer off = new Police_officer(name, code, designation);
                                                officers.Add(off);
                                                officerCount++;
                                        ...
                                        else if (Option == "3")
                                        {
                                            ...
                                            int findOfficerIndex = findIndexForOff(code, officers, officerCount);
                                            int findSignedOfficerIndex = findIndexForSignOff(code, signedUpOffs, signedoffCount);
                                            if (findOfficerIndex != -1 && code != "PC-123" && code != "PC-456" && code != "PC-789")
                                            {
                                                ...
                                                for (int z = findOfficerIndex; z < officerCount; z++)
                                                {
                                                    if (z == officerCount - 1)
                                                    {
                                                        Police_officer off = new Police_officer(null, null,null);
                                                        officers.Insert(z,off);
                                                    }
                                                    else
                                                    {
                                                        string name = officers[z + 1].name;
                                                        string cd = officers[z + 1].code;
                                                        string designation = officers[z + 1].designation;

                                                        Police_officer off = new Police_officer(name, cd, designation);
                                                        officers.Insert(z, off);
                                                ...
                                                if (findSignedOfficerIndex != -1)
                                                {
                                                    ...
                                                    for (int z = findSignedOfficerIndex; z < signedoffCount; z++)
                                                    {
                                                        ...
                                                        {
                                                            SignedUp_Officer offS = new SignedUp_Officer(null, null, null);
                                                            signedUpOffs.Insert(z, offS);
                                                         
                                                        }
                                                        else
                                                        {
                                                            string name = signedUpOffs[z + 1].signedUpOff;
                                                            string cd = signedUpOffs[z + 1].signedOffCodes;
                                                            string passw = signedUpOffs[z + 1].signedUpOffPass;

                                                            SignedUp_Officer offS = new SignedUp_Officer(name, passw, cd);
                                                            signedUpOffs.Insert(z, offS);
                ...
                else if (option == "2")
                {
                    ...
                    if (var3 == "2")
                    {
                        ...
                        bool exist = existance(var1, officers, c, officerCount);
                        if (!exist)
                        {
                        ...
                        else
                        {
                            ...
                            int indexVar = 0;
                            bool check = checkForOfficers(var1, users, countUser, ref indexVar);
                            checking(check, users, var1, var2, var3, countUser);


                            if (check)
                            {
                                SignedUp_Officer signOffs = new SignedUp_Officer(var1, var2, c);
                                signedUpOffs.Add(signOffs);
                                signedoffCount++;
                    ...
                    else if (var3 == "1")
                    {
                        ...
                        {
                            int indx = 0;
                            bool check = checkForOfficers(var1, users, countUser, ref indx);
                            checking(check, users, var1, var2, var3, countUser);
                    ...
                    else if (var3 == "3")
                    {
                        ...
                        // signing-up the user 
                        bool check = signUp(var1, var2, users, countUser);
                        ...
                        if (check)
                        {
                            if (var3 == "3")
                            {
                                Complainant complainant = new Complainant(var1, var2);
        ...
        static void storeAllUsers(List<User> users, ref int CountUser, string fileName)
        {
            string path = Path.Combine("D:\\uniwork\\2nd-Semester\\OOP\\PD\\Business Application", fileName);
            StreamWriter file = new StreamWriter(path, true);


            if (users[CountUser].name != null)
            {
                file.WriteLine(users[CountUser].name + "/" + users[CountUser].password + "/" + users[CountUser].role);
            }
            file.Flush();
            file.Close();
        }

        static void storeSignedOfficers(List<SignedUp_Officer> offs, ref int CountUser, string fileName)
        {
            ...
            {
                if (offs[x].signedUpOff != null)
                {
                    file.WriteLine(offs[x].signedUpOff + "/" + offs[x].signedUpOffPass + "/" + offs[x].signedOffCodes);
                }
            }
            file.Flush();
            file.Close();
        ...
        static void storeComplainants(List<Complainant> comps, ref int CountUser, string fileName)
        {

            string path = Path.Combine("D:\\uniwork\\2nd-Semester\\OOP\\PD\\Business Application", fileName);
            StreamWriter file = new StreamWriter(path, true);
            if (comps[CountUser].complainant != null)
            {
                file.WriteLine(comps[CountUser].complainant + "/" + comps[CountUser].password);
            }
            file.Flush();
            file.Close();
        }

        static void storeAssignedComplainants(List<Complaint_Assign> assigns, ref int CountUser, string fileName)
        {
            ...
            {
                if (assigns[x].assignedCompCodes != null)
                {
                    file.WriteLine(assigns[x].assignedCompCodes + "/" + assigns[x].assignedOffCode);
                }
            }
            file.Flush();
            file.Close();
        ...
        static void storeStatus(List<Status> sts, ref int CountUser, string fileName)
        {
            ...
            for (int x = 0; x < CountUser + 1; x++)
            {
                if (sts[x].status != null)
                {
                    file.WriteLine(sts[x].status + "/" + sts[x].codeStatus);
                }
            }
            file.Flush();
            file.Close();
        ...
        static void storeComplaintDetails(List<Complaint> comps, int[] Complaints, ref int K, string fileName)
        {
            ...
            for (int x = 0; x < K + 1; x++)
            {
                if (comps[x].code != null)
                {
                    //int l = x + 1;
                    file.WriteLine(comps[x].subject + "/" + comps[x].compName + "/" + comps[x].email + "/" + comps[x].contact + "/" + comps[x].address + "/" + comps[x].city + "/" + comps[x].province + "/" + comps[x].date + "/" + comps[x].urgencyLevel + "/" + comps[x].detail + "/" + comps[x].code + "/" + Complaints[x] + "/" + x);
                }
            }
            file.Flush();
            file.Close();
        ...
        static void storeOfficers(List<Police_officer> offs, ref int OfficerCount, string fileName)
        {
            ...
            for (int x = 3; x <= OfficerCount; x++)
            {
                if (offs[x].name != null)
                {
                    file.WriteLine(offs[x].name + "/" + offs[x].code + "/" + offs[x].designation);
                }
            }
            file.Flush();
            file.Close();
        ...
        static void loadAllUsers(List<User> users, ref int CountUser, string fileName)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    User us = new User(parseData(line, 1), parseData(line, 2), parseData(line, 3));
                    users.Add(us);
                    indexing++;
                }
                CountUser = indexing;
        ...
        static void loadSignedOfficers(List<SignedUp_Officer> signedOff, ref int userCount, string fileName)
        {
            ...
            if (File.Exists(path))
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    SignedUp_Officer off = new SignedUp_Officer(parseData(line, 1), parseData(line, 2), parseData(line, 3));
                    signedOff.Add(off);
                    indexing++;
                }
                userCount = indexing;
        ...
        static void loadOfficers(List<Police_officer> offs, ref int OfficerCount, string fileName)
        {
            ...
            if (File.Exists(path))
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    Police_officer off = new Police_officer(parseData(line, 1), parseData(line, 2), parseData(line, 3));
                    offs.Add(off);
                    indexing++;
                }
        ...
        static void loadComplainants(List<Complainant> comps, ref int CountUser, string fileName)
        {
            ...
            if (File.Exists(path))
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    Complainant comp = new Complainant(parseData(line, 1), parseData(line, 2));
                    comps.Add(comp);
                    indexing++;
                }
                CountUser = indexing;
        ...
        static void loadAssignedComplainants(List<Complaint_Assign> assigns, ref int CountUser, string fileName)
        {
            ...
            if (File.Exists(path))
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    Complaint_Assign assign = new Complaint_Assign(parseData(line, 1), parseData(line, 2));
                    assigns.Add(assign);
                    indexing++;
                }
                CountUser = indexing;
        ...
        static void loadStatus(List<Status> sts, ref int CountUser, string fileName)
        {
            ...
            if (File.Exists(path))
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    Status st = new Status(parseData(line, 1), parseData(line, 2));
                    sts.Add(st);
                    indexing++;
                }
                CountUser = indexing;
        ...
        static void loadComplaintDetails(List<Complaint> comps, ref int K, string fileName, string[] NumberAsString, int[] numberAsInteger)
        {
            ...
            if (File.Exists(path))
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    ...
                    Complaint comp = new Complaint(parseData(line, 2), parseData(line, 3), parseData(line, 4), parseData(line, 5), parseData(line, 6), parseData(line, 7), parseData(line, 8), parseData(line, 10), parseData(line, 1), parseData(line, 11),parseData(line, 9));
                    comps.Add(comp);
        ...
        static bool existance(string inputString1, List<Police_officer> officers, string inputString2, int size)
        {
            int c = 0;
            for (int x = 0; x < size; x++)
            {
             
                if (inputString1 == officers[x].name && inputString2 == officers[x].code)
                {
                    c++;
                    break;
                }
        ...
        static string signIn(List<User> users, string user, string pass, int CountUser)
        {
            int c = 0;
            int index = 0;
        ...
        static bool checkForOff(string Var1, List<Complainant> complainants, int CountUser, ref int indexVar)
        {
            int c = 0;
            for (int j = 0; j < CountUser; j++)
            {
                if (Var1 == complainants[j].password)
                {
                    c++;
                    indexVar = j;
                    break;
        ...
        static bool checkForOfficers(string Var1, List<User> users, int CountUser, ref int indexVar)
        {
            int c = 0;
            for (int j = 0; j < CountUser; j++)
            {
        ...
        static void checking(bool check, List<User> users, string Var1, string Var2,string Var3, int CountUser)
        {
            if (!check)
            {
            ...
            {
                User u = new User(Var1, Var2, Var3);
                users.Add(u);
        ...
        static bool signUp(string Var1, string Var2, List<User> users, int CountUser)
        {
            int c = 0;
            for (int j = 0; j < CountUser; j++)
            {
                if (Var1 == users[j].name && Var2 == users[j].password)
                {
                    c++;
                }
            }
        ...
        static int compCompare(string user, string pass, List<Complainant> complainants, int ComplainantCount)
        {
            int c = 0;
            int j = 0;
            ...
            {
                if (user == complainants[j].complainant && pass == complainants[j].password)
                {
                    c++;
                    break;
                }
        ...
        static int codeCompare(string code, List<Complaint> complaints, int compIndex, Complaint complt)
        {
            int c = 0;
            ...
            if (complaints.Count != 0)
            {
                ...
                if (compIndex >= 0 && compIndex < complaints.Count)
                {
                    //Complaint cmp = complaints[compIndex];
                    if (!string.IsNullOrEmpty(complt.code))
                    {

                        for (int x = 0; x < complt.code.Length; x++)
                        {
                            words = "";
                            while (x < complt.code.Length && complt.code[x] != '|')
                            {
                                words += complt.code[x];
                                x++;
                            }
        ...
        static int findIndex(string code, List<Complaint> complaints, int compIndex)
        {
            int c = 0;
            int index = 0;
        ...
        static int findIndexForSignOff(string code, List<SignedUp_Officer> complaints, int compIndex)
        {
            int c = 0;
            int index = 0;
            ...
            for (int x = 0; x < compIndex; x++)
            {
                if (code == complaints[x].signedOffCodes)
                {
                    c++;
                    break;
                }
        ...
        static int findIndexForOff(string code, List<Police_officer> officers, int compIndex)
        {
            int c = 0;
            int index = 0;
        ...
        static int findIndexForOffName(string name, List<Police_officer> officers, int compIndex)
        {
            int c = 0;
            int index = 0;
        ...
        static int findIndexForStatus(string code, List<Status> complaints, int compIndex)
        {
            int c = 0;
            int index = 0;
            ...
            for (int x = 0; x < compIndex; x++)
            {
                if (code == complaints[x].codeStatus)
                {
                    c++;
                    break;
                }
        ...
        static int findIndexForAssign(string code, List<Complaint_Assign> complaints, int compIndex)
        {
            int c = 0;
            int index = 0;
            ...
            for (int x = 0; x < compIndex; x++)
            {
                if (code == complaints[x].assignedCompCodes)
                {
                    c++;
                    break;
                }
        ...
        static string compareIndex(string inputString, List<string> newString, int CodeCompare)
        {
            int index = 0;
            string words;
            string require = "";
        ...
        static string complaintsList(List<Complaint> complaints, int ComplaintCount, int compIndex, ref int loopVar, int x, int y)
        {
            Console.SetCursorPosition(x, y);
            Console.Write("---------------------------------------------------------------------------------------------------------------------------");
            ...
            if (complaints != null)
            {
                //if (complaints[compIndex].code != null)
                //{
                    separateWords(complaints[compIndex].code, newString, x + 13, y);
                    separateWords(complaints[compIndex].subject, newString, x + 28, y);
                    int index = separateWords(complaints[compIndex].date, newString, x + 79, y);
                    separateWords(complaints[compIndex].urgencyLevel, newString, x + 100, y);
        ...
        static int deleteComplaintt(string mainString, int codeIndex)
        {
            List<string> newString = new List<string>();
            StringBuilder main = new StringBuilder(mainString);

            string MiniString = compareIndex(mainString, newString, codeIndex);
            //Console.WriteLine(MiniString);

            int position = mainString.IndexOf(MiniString);
        ...
        static Complaint deleteComplaint(List<Complaint> complaints , int codeIndex, int compIndex)
        {
            // deleting specific subject
            List<string> newString1 = new List<string>();
            StringBuilder main1 = new StringBuilder(complaints[compIndex].subject);

            string MiniString1 = compareIndex(complaints[compIndex].subject, newString1, codeIndex);

            int position1 = complaints[compIndex].subject.IndexOf(MiniString1);

            main1.Remove(position1, MiniString1.Length + 1);
            complaints[compIndex].subject = main1.ToString();

            // deleting specific complainant nane
            List<string> newString2 = new List<string>();
            StringBuilder main2 = new StringBuilder(complaints[compIndex].compName);

            string MiniString2 = compareIndex(complaints[compIndex].compName, newString2, codeIndex);

            int position2 = complaints[compIndex].compName.IndexOf(MiniString2);

            main2.Remove(position2, MiniString2.Length + 1);
            complaints[compIndex].compName = main2.ToString();

            // deleting specific email
            List<string> newString3 = new List<string>();
            StringBuilder main3 = new StringBuilder(complaints[compIndex].email);

            string MiniString3 = compareIndex(complaints[compIndex].email, newString3, codeIndex);

            int position3 = complaints[compIndex].email.IndexOf(MiniString3);

            main3.Remove(position3, MiniString3.Length + 1);
            complaints[compIndex].email = main3.ToString();

            // deleting specific contact
            List<string> newString4 = new List<string>();
            StringBuilder main4 = new StringBuilder(complaints[compIndex].contact);

            string MiniString4 = compareIndex(complaints[compIndex].contact, newString4, codeIndex);

            int position4 = complaints[compIndex].contact.IndexOf(MiniString4);

            main4.Remove(position4, MiniString4.Length + 1);
            complaints[compIndex].contact = main4.ToString();

            // deleting specific address
            List<string> newString5 = new List<string>();
            StringBuilder main5 = new StringBuilder(complaints[compIndex].address);

            string MiniString5 = compareIndex(complaints[compIndex].address, newString5, codeIndex);

            int position5 = complaints[compIndex].address.IndexOf(MiniString5);

            main5.Remove(position5, MiniString5.Length + 1);
            complaints[compIndex].address = main5.ToString();

            // deleting specific city
            List<string> newString6 = new List<string>();
            StringBuilder main6 = new StringBuilder(complaints[compIndex].city);

            string MiniString6 = compareIndex(complaints[compIndex].city, newString6, codeIndex);

            int position6 = complaints[compIndex].city.IndexOf(MiniString6);

            main6.Remove(position6, MiniString6.Length + 1);
            complaints[compIndex].city = main6.ToString();

            // deleting specific province
            List<string> newString7 = new List<string>();
            StringBuilder main7 = new StringBuilder(complaints[compIndex].province);

            string MiniString7 = compareIndex(complaints[compIndex].province, newString6, codeIndex);

            int position7 = complaints[compIndex].province.IndexOf(MiniString7);

            main7.Remove(position7, MiniString7.Length + 1);
            complaints[compIndex].province = main7.ToString();

            // deleting specific detail
            List<string> newString8 = new List<string>();
            StringBuilder main8 = new StringBuilder(complaints[compIndex].detail);

            string MiniString8 = compareIndex(complaints[compIndex].detail, newString8, codeIndex);

            int position8 = complaints[compIndex].detail.IndexOf(MiniString8);

            main8.Remove(position8, MiniString8.Length + 1);
            complaints[compIndex].detail = main8.ToString();

            // deleting specific code
            List<string> newString9 = new List<string>();
            StringBuilder main9 = new StringBuilder(complaints[compIndex].code);

            string MiniString9 = compareIndex(complaints[compIndex].code, newString9, codeIndex);

            int position9 = complaints[compIndex].code.IndexOf(MiniString9);

            main9.Remove(position9, MiniString9.Length + 1);
            complaints[compIndex].code = main9.ToString();

            // deleting specific urgency level
            List<string> newString10 = new List<string>();
            StringBuilder main10 = new StringBuilder(complaints[compIndex].urgencyLevel);

            string MiniString10 = compareIndex(complaints[compIndex].urgencyLevel, newString10, codeIndex);

            int position10 = complaints[compIndex].urgencyLevel.IndexOf(MiniString10);

            main10.Remove(position10, MiniString10.Length + 1);
            complaints[compIndex].urgencyLevel = main10.ToString();

            // deleting specific date
            List<string> newString11 = new List<string>();
            StringBuilder main11 = new StringBuilder(complaints[compIndex].date);

            string MiniString11 = compareIndex(complaints[compIndex].date, newString11, codeIndex);

            int position11 = complaints[compIndex].date.IndexOf(MiniString11);

            main11.Remove(position11, MiniString11.Length + 1);
            complaints[compIndex].date = main11.ToString();

            return complaints[compIndex];

        }

        static int editComplaint(List<string> mainString, int codeIndex, int compIndex)
        {
            List<string> newString = new List<string>();
        ...
        static void storeInMain(string editedString, List<string> mainArr,List<Complaint> complaints, int ComplaintCount)
        {
            if (editedString == "subject")
            {
                for (int i = 0; i <= ComplaintCount; i++)
                {
                    complaints[i].subject = mainArr[i];
                }
            ...
            else if (editedString == "name")
            {
                for (int i = 0; i <= ComplaintCount; i++)
                {
                    complaints[i].compName = mainArr[i];
                }
            ...
            else if (editedString == "contact number")
            {
                for (int i = 0; i <= ComplaintCount; i++)
                {
                    complaints[i].contact = mainArr[i];
                }
            ...
            else if (editedString == "home address")
            {
                for (int i = 0; i <= ComplaintCount; i++)
                {
                    complaints[i].address = mainArr[i];
                }
            ...
            else if (editedString == "province")
            {
                for (int i = 0; i <= ComplaintCount; i++)
                {
                    complaints[i].province = mainArr[i];
                }
            ...
            else if (editedString == "urgency level")
            {
                for (int i = 0; i <= ComplaintCount; i++)
                {
                    complaints[i].urgencyLevel = mainArr[i];
                }
            ...
            else if (editedString == "detail")
            {
                for (int i = 0; i <= ComplaintCount; i++)
                {
                    complaints[i].detail = mainArr[i];
                }
                
            }
        }

        static string findMainString(string number, List<Complaint> complaints, int codeIndex, int index, int ComplaintCount, List<string> mainArr, ref string editedString)
        {
            List<string> newString = new List<string>();
            string mainString = "";
            if (number == "1")
            {
                mainString = compareIndex(complaints[index].subject, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i,complaints[i].subject);
                }
            ...
            {
                mainString = compareIndex(complaints[index].code, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].code);
            ...
            {
                mainString = compareIndex(complaints[index].compName, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].compName);
            ...
            {
                mainString = compareIndex(complaints[index].email, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].email);
            ...
            {
                mainString = compareIndex(complaints[index].contact, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].contact);
            ...
            {
                mainString = compareIndex(complaints[index].address, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].address);
            ...
            {
                mainString = compareIndex(complaints[index].city, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].city);
            ...
            {
                mainString = compareIndex(complaints[index].province, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].province);
            ...
            {
                mainString = compareIndex(complaints[index].date, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].date);
            ...
            {
                mainString = compareIndex(complaints[index].urgencyLevel, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].urgencyLevel);
            ...
            {
                mainString = compareIndex(complaints[index].detail, newString, codeIndex);

                for (int i = 0; i < ComplaintCount; i++)
                {
                    mainArr.Insert(i, complaints[i].detail);
        ...
        static void generalDetails(List<Complaint> complaints, int codeIndex, int compIndex, int x, int y)
        {
            Console.SetCursorPosition(x, y);
            Console.Write("--------------------------------------------------------------------------------------------------------------");
            Console.SetCursorPosition(x, y + 1);
            ...
            List<string> newString = new List<string>();
            Console.SetCursorPosition(x, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].code, newString, codeIndex));
            Console.SetCursorPosition(x + 15, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].subject, newString, codeIndex));
            Console.SetCursorPosition(x + 66, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].date, newString, codeIndex));
            Console.SetCursorPosition(x + 87, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].urgencyLevel, newString, codeIndex));
        ...
        static void personalDetails(List<Complaint> complaints, int codeIndex, int compIndex, int x, int y)
        {
            Console.SetCursorPosition(x, y);
            Console.Write("---------------------------------------------------------------------------------------------------------------------------");
            ...
            List<string> newString = new List<string>();
            Console.SetCursorPosition(x, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].compName, newString, codeIndex));
            Console.SetCursorPosition(x + 19, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].email, newString, codeIndex));
            Console.SetCursorPosition(x + 47, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].contact, newString, codeIndex));
            Console.SetCursorPosition(x + 70, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].address, newString, codeIndex));
            Console.SetCursorPosition(x + 96, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].city, newString, codeIndex));
            Console.SetCursorPosition(x + 109, y + 3);
            Console.Write("| " + compareIndex(complaints[compIndex].province, newString, codeIndex));
            ...
            Console.SetCursorPosition(x + 3, y + 8);
            Console.Write(compareIndex(complaints[compIndex].detail, newString, codeIndex));
        }

        static Complaint complaint(List<Complaint> complaints, int K, List<Status> status, int StatusCount, int compCount)
        {
            Complaint complaint;
            if (compCount > 0)
            {
                complaint = complaints[K];
            }
            else 
            {
            ...
            int indexCheck = codeCompare(code, complaints, K, complaint);
            while (indexCheck != -1)
            {
                Console.SetCursorPosition(x, y);
                ...
                Console.SetCursorPosition(x, y);
                Console.Write("                                                              ");
                indexCheck = codeCompare(code, complaints, K, complaint);
            }
            ...
            Status st = new Status("Pending",code);
            status.Add(st);
            

           
            complaint.compName += name + "|";
            complaint.email += email + "|";
            complaint.contact += contact + "|";
            complaint.address += addr + "|";
            complaint.city += city + "|";
            complaint.province += province + "|";
            complaint.date += date + "|";
            complaint.detail += detail + "|";
            complaint.code += code + "|";
            complaint.subject += sub + "|";
            complaint.urgencyLevel += urgent + "|";

            //complaints.Insert(K,complaint);
        ...
        static string complainantsAdmin(List<Complainant> complainants, int ComplainantCount)
        {
            int x = 41, y = 20;
            Console.SetCursorPosition(x, y);
            Console.Write("-----------------------------------------------------------------------------------------------");
            ...
            while (i < ComplainantCount)
            {
                ...
                Console.Write("|       " + complainants[i].complainant);
                Console.SetCursorPosition(106, 23 + i);
                Console.Write("|      " + complainants[i].password);
        ...
        static int officersList(List<Police_officer> officers, int OfficerCount, int x, int y)
        {
            Console.SetCursorPosition(x, y);
            Console.Write("-----------------------------------------------------------------------------------------------");
            Console.SetCursorPosition(x, y + 1);
            ...
            while (i < OfficerCount)
            {
                ...
                Console.Write("|     " + officers[i].name);
                Console.SetCursorPosition(92, y + 3 + i);
                Console.Write("|     " + officers[i].designation);
                Console.SetCursorPosition(124, y + 3 + i);
                Console.Write("|  " + officers[i].code);
        ...
        static int signedOfficersList(List<SignedUp_Officer> signedOff, List<Police_officer> officers, int signedOfficerCount, int OfficerCount, int x, int y)
        {
            ...
            else
            {
                ...
                while (i < signedOfficerCount && signedOff[i].signedUpOff != null)
                {

                    int index = findIndexForOffName(signedOff[i].signedUpOff, officers, OfficerCount);

                    Console.SetCursorPosition(41, y + 3 + i);
                    int l = i + 1;
                    Console.Write("|       " + l);
                    Console.SetCursorPosition(57, y + 3 + i);
                    Console.Write("|     " + signedOff[i].signedUpOff);
                    Console.SetCursorPosition(92, y + 3 + i);
                    Console.Write("|     " + officers[index].designation);
                    Console.SetCursorPosition(124, y + 3 + i);
                    Console.Write("|  " + officers[index].code);
                    Console.SetCursorPosition(135, y + 3 + i);
                    Console.Write("|");
        ...
        static int findAllIndexes(string code, string[] Codes, int compIndex, List<int> indexes)
        {
            int c = 0;
            int index = 0;
            for (int x = 0; x < compIndex; x++)
        ...
        static int findAllIndexesAssigned(string code, List<Complaint_Assign> assignComps, int compIndex, List<int> indexes)
        {
            int c = 0;
            int index = 0;
            for (int x = 0; x < compIndex; x++)
            {
                ...
                if (code == assignComps[x].assignedOffCode)
                {
                    indexes.Add(x);
                    c++;
                }
        ...
        static void me(string user, List<Police_officer> officers, int OfficerCount)
        {
            int indexForMe = findIndexForOffName(user, officers, OfficerCount);
            ...
            Console.Write("               My Designation: " + officers[indexForMe].designation);
            Console.SetCursorPosition(x + 23, y + 9);
            Console.Write("               My Code: " + officers[indexForMe].code);
        ...
        static string complaintsListForAssign(List<Complaint> complaints, int ComplaintCount, int compIndex, int numberOfIndexes, ref int loopVar, int x, int y, int j, ref int serial)
        {
            string[] newString = new string[500];

            if (complaints[compIndex].code != null)
            {
                separateWordsForAssign(ComplaintCount, complaints[compIndex].code, newString, x + 13, y);
                separateWordsForAssign(ComplaintCount, complaints[compIndex].subject, newString, x + 28, y);
                int index = separateWordsForAssign(ComplaintCount, complaints[compIndex].date, newString, x + 79, y);
                separateWordsForAssign(ComplaintCount, complaints[compIndex].urgencyLevel, newString, x + 100, y);
